...
export const useBlankCheckHandlers = (
  ...
  const handleIncorrectBlanksAnswer = () => {
    ...
    // Reset only the blank fields, keeping the non-blank characters
    // This allows the user to try again on the same stage
    const resetBlanks = word.text.split('').map((char, i) => 
      blankIndices.includes(i) ? '' : char
    );
    setBlanksAttempt(resetBlanks);
    setManuallyChanged(false); // Reset the flag when blanks are reset
    playPronunciationTimes(1);  // Play once to remind the user of the word pronunciation
    startTimer();
  };
