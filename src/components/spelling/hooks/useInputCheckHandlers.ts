...
export const useInputCheckHandlers = (
  ...
  const handleIncorrectSpelling = () => {
    ...
    // Trigger the incorrect attempt animation
    ...
    // Stay on the current stage (input) instead of going back to the full word view
    // This allows the user to try again on the same stage
    setLetterAttempts(Array(word.text.length).fill(''));
    // Play once to remind the user of the word pronunciation
    playPronunciationTimes(1);
    startTimer();
  };
